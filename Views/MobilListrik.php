<?php
$title = "Daftar Mobil Listrik - Sistem Kendaraan";
...
$result = $database->tampilDatabaseMobilListrik();
$daftarMobilListrik = [];

while ($row = mysqli_fetch_assoc($result)){
    $daftarMobilListrik[]= new MobilListrik(
        $row['id_kendaraan'],
        $row['brand'],
        $row['model'],
        $row['tahun'],
        $row['harga_dasar'],
        $row['kapasitas_baterai'],
        $row['jarak_tempuh']
    );
}
    ...
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            ...
            <p class="mt-1 text-xs text-gray-700 dark:text-gray-300">
                Daftar mobil Listrik yang dikelola melalui implementasi abstract class dan database.
            </p>
        </div>
    </div>
        ...
        <div class="-mx-3 -my-2 overflow-x-auto sm:-mx-5 lg:-mx-6">
            <div class="inline-block min-w-full py-2 align-middle">
                <table class="min-w-full border-separate border-spacing-0">
                    ...
                    <tbody>
                        <?php if (!empty($daftarMobilListrik)) : ?>

                            <?php
                            $totalData = count($daftarMobilListrik);

                            foreach ($daftarMobilListrik as $index => $m) :
